...
def is_gender_and_relationship_match(user_preferences, user_by_distance_preferences):
    return (
        user_preferences.interested_in == user_by_distance_preferences.interested_in
        and user_preferences.relationship == user_by_distance_preferences.relationship
    )
...
def filter_profiles(current_user, user_preferences, nearby_users):
    ...
    for nearby_user in nearby_users:
        ...
        if (
            nearby_user in current_user_blocked_users
            or nearby_user in current_user_liked_users
        ):
            continue
        ...
        nearby_user_age_range = (
            nearby_user_preferences.age_min,
            nearby_user_preferences.age_max,
        )
        ...
        if (
            is_age_within_range(nearby_user_age, *current_user_age_range)
            and is_age_within_range(current_user_age, *nearby_user_age_range)
            and nearby_user_distance <= nearby_user_distance_preference
            and is_gender_and_relationship_match(
                user_preferences, nearby_user_preferences
            )
        ):
